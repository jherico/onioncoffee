...
class HTTPConnection extends Thread {
    ...
    private static final int payload = 498; // maximum dataLength of Tor-Cell
                                            // Payload

    private Tor tor;
    public HTTPConnection(Socket local, Tor tor) {
        // start connection
        this.tor = tor;
        this.local = local;
    ...
    private String FilterAndReplace(String input) {
        if (!MainWindow.processHTTPHeaders()) {
            return input + "\r\n";
        }
        try {
            ...
            for (int i = 0; (i < Config.setFilteredHeaders.size()) && (!filtered); ++i) {
                String conf = (Config.setFilteredHeaders.elementAt(i)).toLowerCase();
            ...
            if (name.equalsIgnoreCase("Cookie")) {
                ...
                if ((idxRoute + 6 <= idxTorJava) && (idxRoute + 8 >= idxTorJava)) {
                    int nextQM = value.indexOf("\"", idxTorJava);
                    System.err.println("filter out: " + idxRoute + " " + idxTorJava + " " + nextQM + " " + value);
                    if (nextQM < 0) {
            ...
            // substitute?
            for (int i = 0; i < Config.setReplaceHeaders.size(); ++i) {
                String[] replace = Config.setReplaceHeaders.elementAt(i);
    ...
    public void run() {
        ...
        try {
            ...
            BufferedReader br = new BufferedReader(new InputStreamReader(local.getInputStream()));
            ...
            while (header.length() > 0) {
                if (header.startsWith("Host: ")) {
                    ...
                    // System.out.println("Overwriting host with "+host+" from '"+header+"'");
                    if (host.indexOf(':') >= 0) {
                        try {
            ...
            if (protocol == PROTOCOL_HTTP) {
                // write http-request
                DataOutputStream remote_write = new DataOutputStream(remoteOutput);
                remote_write.write(request.toString().getBytes());
                remote_write.flush();
                // relay http-response - HEADER
                BufferedWriter local_write = new BufferedWriter(new OutputStreamWriter(local.getOutputStream()));
                ...
                DataOutputStream local_write = new DataOutputStream(local.getOutputStream());
                local_write.write(success200.getBytes());
                local_write.flush();
                // relay subsequent data
                SocksConnection.relay(local, new DataInputStream(remoteInput), new DataOutputStream(remoteOutput));
            }
        }
